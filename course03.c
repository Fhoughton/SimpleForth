/* Forth interpreter and simple compiler (no conditionals) - in C
 * (c) 2017 Andreas Klimas, klimas@w3group.de
 *
 * This source is public domain
 *
 * We have a datastack, a dictionary, and string handling
 * now we need a return stack and a macro dictionary, code segment
 * and an instruction pointer
 *
 * The big deal now is that we must have a virtual machine
 * running where the interpreter and compiler is running inside.
 * So we have to compile our first word (I name it shell) by hand
 */
...
static xt_t *xt_dup, *xt_drop, *xt_interpreting, *xt_word, *xt_bye, *xt_lit, *xt_leave, *xt_branch, *xt_0branch; // some execution tokens need for compiling
...
static void f_docol(void) { // course03, VM: enter function (word)new 
	rp_push(ip); //push current ip to stack to jump to function and recover later
	ip=current_xt->data; //set the ip to the body of current_xt and execute it! this means it finds the new definition and runs the data section assigned to it, which contains the code to be run of the function!
}
